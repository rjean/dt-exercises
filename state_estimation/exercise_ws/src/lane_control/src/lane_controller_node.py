...
class LaneControllerNode(DTROS):
    ...
    def cbGroundProjectedLineSegments(self, segments_msg):
        ...
        lookup_distance = rospy.get_param("lookup_distance",0.3)
        ...
        car_control_msg.v = rospy.get_param("speed",0.7)
        alpha = np.arctan(aim_point[1]/aim_point[0])
        car_control_msg.omega = np.sin(alpha) / rospy.get_param("K",0.15)

        if abs(car_control_msg.omega) > rospy.get_param("turn_th",0.35):
            car_control_msg.v = rospy.get_param("turn_speed",0.25)
